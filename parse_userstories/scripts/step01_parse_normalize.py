...
Usage:
    python step01_parse_normalize.py requirements.pdf
    (will automatically read from data/requirements.pdf)
"""
...
def normalize_text(raw: str) -> str:
    ...
    text = re.sub(HEADING_PATTERN, lambda m: tidy_heading(m), text)
    return text.strip()
...
def main(argv=None):
    ...
    parser.add_argument("filename", help="File name inside data/ folder, e.g. requirements.pdf")
    args = parser.parse_args(argv)

    # Always read from data folder
    input_path = os.path.join("data", args.filename)
    if not os.path.exists(input_path):
        raise FileNotFoundError(f"File not found: {input_path}")

    loader = _detect_loader(input_path)
    raw = loader(input_path)
    ...
    raw_path = os.path.join(out_dir, "01_raw.txt")
    norm_path = os.path.join(out_dir, "01_normalized.txt")
